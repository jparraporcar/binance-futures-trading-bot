...
class macd_peak_long_trader():
    ...
    def __init__(self, symbol=None, units='0.0006', interval=None, ema_slow=None, ema_fast=None, ema_signal=None, sesion_end=None, testnet=None, assigned_duration_minutes=None, emergency_price_chg_pct=None):
        ...
        self.cum_profits = None #Final profit at the end of the trading sesion where the latest position is neutral
    ...
    def stop_ses(self, save_to_file=True):
        ...
        if self.position == None:
            if (len(self.trade_values) == 0):
                if (self.emergency_price_chg == True): ###CHECKED-OK###
...
#       ["1m", "3m", "5m", "15m", "30m", "1h", "2h", "4h", "6h", "8h", "12h", "1d", "3d", "1w", "1M"]
    ...
    def save_to_file(self):
        file_name = f"macd_peak__symbol_{self.symbol}__interval_{self.interval}__eslow_{self.ema_slow}_efast_{self.ema_fast}_esign_{self.ema_signal}__duration_{self.run_end_delta}min_upto_{self.assigned_duration_minutes}min__profit_{self.cum_profits}dollar__tradesnum_{self.trades_num}" 
