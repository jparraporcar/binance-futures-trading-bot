...
class Macd_ls_backtester_sma():
    ...
    def execute_backtest(self, start=None, ema_slow=None, ema_fast=None, ema_signal=None, sma_slow=None, sma_fast=None):
        '''
        ...
        for index, data in self.data_init.iterrows():
            ...
            if ((data.macd_inv_sign == 1 and data.sma_inv_sign == 1) or (data.macd_inv_sign == 1 and data.sma_conf == 1)): #CHECKED
                ...
                for index_sub_buy, data_sub_buy in self.data_init_sub_buy.iterrows():
                    ...
                    if ((data.macd_inv_sign == -1 and data.sma_inv_sign == -1) or (data.macd_inv_sign == -1 and data.sma_conf == -1) or (data.sma_inv_sign == -1 and data.macd_diff_conf == -1)):
                        break                    
                    else: 
                ...
                for index_sub_buy, data_sub_sell in self.data_init_sub_sell.iterrows():
...
#         close_ax.xaxis.set_major_formatter(date_form)
    ...
    def execute_opt(self, start_opt=None, end_opt=None, ema_slow_opt=None, ema_fast_opt=None, ema_sign_opt=None, sma_slow=None, sma_fast=None):
        '''
        REMARK: Introduced time must be in Tokyo time (UTC+9) but the calculations will be in UTC
        '''
        ...
        sma_slow_opt = range(*sma_slow)
        sma_fast_opt = range(*sma_fast)
        combinations = list(product(macd_slow_opt, macd_fast_opt, macd_signal_opt, sma_slow_opt, sma_fast_opt))
        ...
        for comb in combinations:
            try:
                ...
                tuple_results = (comb[0], comb[1], comb[2], comb[3], comb[4], *self.execute_backtest(start=self.start, ema_slow=comb[0], ema_fast=comb[1], ema_signal=comb[2], sma_slow=comb[3], sma_fast=comb[4]), start_opt, end_opt)
        ...
        mg = pd.DataFrame(data=results, columns = ['macd_slow_opt', 'macd_fast_opt', 'macd_signal_opt', 'sma_slow', 'sma_fast', 'multiple_hold', 'ann_log_mean_hold', 'ann_log_std_hold', 'sharpe_ratio_hold', 'multiple_macd_strategy', 'ann_log_mean_macd', 'ann_log_std_macd', 'sharpe_ratio_macd', 'multiple_macd_strategy_net', 'ann_log_mean_macd_net', 'ann_log_std_macd_net', 'sharpe_ratio_macd_net', 'success_trades_net', 'failure_trades_net', 'success_trades_net_pct', 'failure_trades_net_pct', 'total_trades', 'start_opt', 'end_opt'])
        ...
        if os.path.exists(f"macd_ls_sma_{self.opt_ini_time_str}_{self.interval}_{self.symbol}.csv"):
            df_imported = pd.read_csv(f"macd_ls_sma_{self.opt_ini_time_str}_{self.interval}_{self.symbol}.csv")
            df_complete = pd.concat([df_imported, self.opt_results], axis = 0)
            df_complete.to_csv(f"macd_ls_sma_{self.opt_ini_time_str}_{self.interval}_{self.symbol}.csv", mode='w', index=False, header=True)
        else:
            self.opt_results.to_csv(f"macd_ls_sma_{self.opt_ini_time_str}_{self.interval}_{self.symbol}.csv", mode='w', index=False, header=True)    
