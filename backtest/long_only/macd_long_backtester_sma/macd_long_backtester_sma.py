...
class Macd_long_backtester_sma():
    '''
        ...
        for index, data in self.data_init.iterrows():
            ...
            if (data.inv_sign == 1 and data.sma_conf == 1):
                ...
                for index_sub_buy, data_sub_buy in self.data_init_sub_buy.iterrows():
                    if (data_sub_buy.inv_sign == 0 and data_sub_buy.sma_conf == 1):
                        self.data_init.loc[index_sub_buy, 'position'] = 1
                    elif (data_sub_buy.inv_sign == -1 and data_sub_buy.sma_conf == 1):
                        self.data_init.loc[index_sub_buy, 'position'] = 1
                    elif (data_sub_buy.sma_conf != 1):
                        break
                    pass 
            else:
                pass
...
#         close_ax.xaxis.set_major_formatter(date_form)
    ...
    def execute_opt(self, start_opt=None, end_opt=None, ema_slow_opt=None, ema_fast_opt=None, ema_sign_opt=None, sma_slow=None, sma_fast=None):
        ...
        if os.path.exists(f"macd_sma_{self.opt_ini_time_str}_{self.interval}.csv"):
            df_imported = pd.read_csv(f"macd_sma_{self.opt_ini_time_str}_{self.interval}.csv")
            df_complete = pd.concat([df_imported, self.opt_results], axis = 0)
            df_complete.to_csv(f"macd_sma_{self.opt_ini_time_str}_{self.interval}.csv", mode='w', index=False, header=True)
        else:
            self.opt_results.to_csv(f"macd_sma_{self.opt_ini_time_str}_{self.interval}.csv", mode='w', index=False, header=True)    
